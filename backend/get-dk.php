...
# in:JSON array of game info data 
# out: JSON Players from selected games
# query: "SELECT * FROM `dk05s` WHERE `GameInfo`IN ("GameInfo", "GameInfo"...);" 
...
function getFilteredPlayerPool($query){ // $query = array of games : built to string
  global $conn;
  $sql = "SELECT * FROM `dk05s` WHERE `GameInfo` IN $query;";
  //
  $dbData = mysqli_query($conn, $sql);
    // check for no data
  if(mysqli_num_rows($dbData) < 1){ 
